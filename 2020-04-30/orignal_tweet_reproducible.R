...
library(ggplot2) # requires currently a dev version of ggplot
...
showlen <- grosses %>%
  group_by(interaction(show, theatre)) %>% # this could be refactored
  summarise(show = show[1], # this is a side effect of the interaction use above
            theatre = theatre[1],
...
timeplot <- ggplot(shows_filt, 
  ...
  #                    expand = c(0,0),
  #                    guide = "axis_logticks") +
  scale_y_continuous(name = "Average Ticket Price",
                   limits = c(20, 300),
                   expand = c(0,0)) +
  scale_x_date(name = "") +
...
startend <- 
  ...
  geom_segment(
    aes(x = 1, xend = 2,
        ...
        alpha = log(n),
        alt_colour = end_price - start_price) # alt_colour
  ) +
