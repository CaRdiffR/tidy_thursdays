...
bsongperf <- billboard %>% group_by(song_id) %>% slice_min(peak_position, with_ties = FALSE)
...
top_song_features <- bsongperf %>% left_join(audio_features, by = "song_id")

features_by_year <- top_song_features %>% group_by(year=floor_date(week_id, "year")) %>%
  summarize(
